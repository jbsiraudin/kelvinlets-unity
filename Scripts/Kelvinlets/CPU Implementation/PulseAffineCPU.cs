...
namespace Kelvinlet
{
    public class PulseAffineCPU : MonoBehaviour
    {
        Vector3 _pos;
        public float3x3 _affineForce = float3x3.zero;
        Type _type = Type.Pinch;
        ...
        public void SetBrush(Type t, Vector3 point, float3x3 f, float eps, float maxTime, float minValue, float maxValue)
        {
            _type = t;
            _pos = point;
            _affineForce = f;
        ...
        Vector3 EvalDisp(Vector3 query, float time)
        {
            ...
            float3 Fx = math.mul(_affineForce, x);
            float trace = _affineForce[0][0] + _affineForce[1][1] + _affineForce[2][2];
            float3 TrTr = math.mul(math.transpose(_affineForce), x) + trace * x;
